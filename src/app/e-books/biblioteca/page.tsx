...
import { Search, Headphones, Play, Star, Clock, Volume2, Book } from "lucide-react"
...
export default function ebooksLibraryPage() {
  ...
  // Dados dos ebooks em destaque (mantidos)
  const featuredEbooks = [
    { title: "Fundamentos da Fisioterapia Ortopédica", writer: "Dra. Georgia Espinola Moura", category: "Ortopedia", duration: "4h 32min", rating: 4.9, listeners: 850, image: "/fisio3.png", premium: true },
    { title: "Neuroplasticidade e Reabilitação", writer: "Dra. Georgia Espinola Moura", category: "Neurologia", duration: "3h 45min", rating: 4.8, listeners: 620, image: "/fisio2.png", premium: true },
    { title: "Técnicas de Respiração Terapêutica", writer: "Dra. Georgia Espinola Moura", category: "Respiratória", duration: "2h 18min", rating: 4.7, listeners: 490, image: "/fisio6.png", premium: false },
  ]
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-center mb-6">
               <div className="w-20 h-20 bg-yellow/20 backdrop-blur-sm rounded-full flex items-center justify-center">
                <Image src="/ebook.png" alt="Ícone de fones de ouvido" width={70} height={70} />
              </div>
            </div>
            ...
            <p className="text-base md:text-xl leading-relaxed mb-12">
              Planos feitos sob medida para o seu bolso, pensados para transformar seu dia a dia! Acreditamos que cuidar de você não precisa custar caro.
            </p>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">Conteúdo escrito por profissionais especializados em cada área.</p>
          </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featuredEbooks.map((ebook) => (
              <Card key={ebook.title} className="overflow-hidden rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300 border">
                <div className="relative h-56">
                  <Image src={ebook.image} alt={ebook.title} fill className="object-cover" />
                  {ebook.premium && <Badge className="absolute top-4 right-4 bg-golden-500 text-black shadow-md">Premium</Badge>}
                </div>
                <CardContent className="p-6">
                  <Badge variant="outline" className="mb-2">{ebook.category}</Badge>
                  <h3 className="text-xl font-semibold mb-2 h-14">{ebook.title}</h3>
                  <p className="text-gray-500 text-sm mb-4">escrito por {ebook.writer}</p>
                  <div className="flex items-center justify-between text-sm text-gray-600 mb-6">
                    <div className="flex items-center space-x-2"><Clock className="w-4 h-4" /><span>{ebook.duration}</span></div>
                    <div className="flex items-center space-x-2"><Star className="w-4 h-4 text-yellow-400" /><span>{ebook.rating}</span></div>
                    <div className="flex items-center space-x-2"><Book className="w-4 h-4" /><span>{ebook.listeners}</span></div>
                  </div>
